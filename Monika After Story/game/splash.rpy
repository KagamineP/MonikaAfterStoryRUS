...
init -100 python:
    ...
    for archive in ['audio','images','scripts','fonts']:
        if not archive in config.archives:
            ...
            renpy.error("Архивные файлы DDLC не найдены в папке /game. Проверьте установку и повторите попытку.")
...
init python:
    ...
    splash_message_default = _("Эта игра является неофициальной работой фанатов, не связанной с Team Salvato.")
    splash_messages = [
...
label splashscreen:
    ...
    if not persistent.first_run:
        ...
        with Dissolve(1.0)
        pause 1.0
        "[config.name] - это фан-мод Doki Doki Literature Club, который не связан с Team Salvato."
        "Он предназначен для игры только после завершения официальной игры и содержит спойлеры для официальной игры."
        "Файлы игры для Doki Doki Literature Club необходимы для игры в этот мод и могут быть загружены бесплатно по адресу: http://ddlc.moe"
        menu:
            "Играя в [config.name] вы соглашаетесь с тем, что вы завершили Doki Doki Literature Club и принимаете любые спойлеры, содержащиеся в нем."
            "Я согласен.":
                pass
        scene tos2
        with Dissolve(1.5)
...
label after_load:
    ...
    if anticheat != persistent.anticheat:
        stop music
        scene black
        "Файл сохранения не удалось загрузить."
