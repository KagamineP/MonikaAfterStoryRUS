...
###############################################################################
    ...
    def _init_talk_quips():
        ...
        quips = [
            ...
            _("...А?"),
            _("...Хм?"),
            _("Наверное, мы можем попробовать поговорить."),
            _("Наверное, мы можем поговорить."),
            _("О... Ты хочешь поговорить?"),
            _("Если ты хочешь поговорить, давай."),
            _("Мы можем поговорить, если ты действительно хочешь."),
            ...
            _("Ты уверен[mas_gender_none], что хочешь поговорить?")
        ]
...
#            "Just...whatever, go ahead."
        ...
        ## NORMAL quips
        quips = [
            _("О чем бы ты хотел[mas_gender_none] поговорить?"),
            _("Есть что-то, о чем ты хотел[mas_gender_none] бы поговорить?")
        ]
        ...
        quips = [
            ...
            _("О чём ты хочешь поговорить, [player_abb]?"),
            _("Да, [player]?"),
            _("Что у тебя на уме, [player]?"),
            _("О чём ты хочешь поговорить, [player]?")
        ]
        ...
        quips = [
            _("О чём ты бы хотел[mas_gender_none] поговорить? <3"),
            _("О чем бы ты хотел[mas_gender_none] поговорить, мил[mas_gender_iii] ?"),
            _("Да, мил[mas_gender_iii]?"),
            _("Да, солнышко?"),
            _("Да, дорог[mas_gender_oi]?"),
            _("Что у тебя на уме, дорог[mas_gender_oi]?"),
            _("О чем бы ты хотел[mas_gender_none] поговорить, мил[mas_gender_iii]?"),
            _("О чем бы ты хотел[mas_gender_none] поговорить, [player]?"),
            _("Да, [player]?"),
            _("Что у тебя на уме, [player]?"),
            _("О чем бы ты хотел[mas_gender_none] поговорить, [player]?")
        ]
...
#            "Hey, what's up?",
            ...
            _("Что у тебя на уме, дорог[mas_gender_oi]?"),
            _("Что-нибудь на уме?"),
            _("Что-нибудь на уме, мил[mas_gender_iii]?"),
            _("Как дела, мил[mas_gender_iii]?"),
            _("Как дела, солнышко?"),
            _("Как дела, дорог[mas_gender_oi]?"),
            _("Как дела, мил[mas_gender_iii]?"),
            _("Как дела, [player]?"),
#            "What's up?",
            _("Да, мил[mas_gender_iii]?"),
            _("Да, солнышко?"),
            _("Да, дорог[mas_gender_oi]?"),
            _("Да, любим[mas_gender_iii]?"),
            _("^_^"),
            _("<3"),
            _("Хочешь о чем-нибудь поговорить?"),
            _("Мы можем поговорить о чём угодно, [player_abb].")
        ]
    ...
    def mas_setApologyReason(
        ...
        if ev_label is None:
            ...
            store.mas_utils.writelog(
                "[ОШИБКА]: ev_label не существует: {0}\n".format(repr(ev_label))
            )
            return
...
label monika_affection_nickname:
    ...
    if not persistent._mas_offered_nickname:
        ...
        m 3hua "Но последнее слово все равно останется за мной!"
        m "Что ты на это скажешь?{nw}"
        python:
            # change the prompt for this event
            aff_nickname_ev.prompt = _("Могу я называть тебя по-другому?")
            ...
            while not done:
                python:
                    inputname = mas_input(
                        ...
                        allow=" abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZабвгдеёжзийклмнопрстуфхчшщцьыъэюяАБВГДЕЁЖЗИЙКЛМНОПРСТУФХЧШЩЦЬЫЪЭЮЯ-_",
                        length=14,
                ...
                if lowername == "cancel_input":
                    m 1euc "А, понятно."
                    m 1tkc "Что ж...какая жалость."
                    m 3eka "Но это нормально. Мне все равно нравится «[m_name]»."
                    $ done = True

                elif not lowername:
                    m 1lksdla "..."
                    m 1hksdrb "Ты долж[mas_gender_en] дать мне имя, [player]!"
                    m "Клянусь, временами ты так глуп[mas_gender_none]."
                    m 1eka "Попробуй еще раз!"
                    ...
                    m 1euc "..."
                    m 1hksdlb "Я думала, мы выбираем новое имя, глупышка."
                    m 1eka "Попробуй еще раз~"

                elif re.findall("mon(-|\\s)+ika", lowername) or re.findall("мон(-|\\s)+ика", lowername):
                    m 2tfc "..."
                    m 2esc "Попробуй еще раз."
                    show monika 1eua
                    ...
                    m 1rkc "..."
                    m 1rksdld "Хотя я и не ненавижу это, но не думаю, что мне удобно, когда ты называешь меня так."
                    ...
                    if not mas_bad_name_comp.search(inputname) and lowername not in ["yuri", "sayori", "natsuki"]:
                        if inputname == "Monika":
                            ...
                            m 1wuo "О! Какое замечательное имя!"
                            ...
                            m 1duu "[inputname]... Это довольно милое имя."
                            m 3ekbsa "Спасибо, [player]. Ты так[mas_gender_oi] мил[mas_gender_iii]!~"
                        ...
                        if monika_name == "Monika" or monika_name == "Моника" or monika_name == "monika" or monika_name == "моника":
                            ...
                            m 3hua "С этого момента ты можешь звать меня «[m_name].»"
                            m 1hua "Ehehe~"
                        $ done = True
